...
公网访问媒体服务器播放视频时，自动对下载器设置上传限速；内网播放或没有观看时解除上传限速，或恢复到你设置的另一个上传限速。

核心行为：
- 默认把局域网、回环、链路本地地址视为不限速客户端，公网客户端触发限速。
- 支持填写 IPv4/IPv6 CIDR 白名单；命中白名单的播放客户端不触发限速。
- 支持按媒体路径片段排除，例如儿童目录、纪录片目录等。
- 支持固定公网播放上传限速，或按上行总带宽减去当前公网视频码率后自动计算上传限速。
- 支持选择一个或多个下载器；多下载器智能限速可平均分配或按比例分配。
- “公网播放下载限速”留空时不修改下载速度，保持下载器自己的设置；填写后只在公网播放期间设置下载限速，结束后恢复原下载限速。
- 插件每次检查和状态切换都会写入日志，多个下载器的通知会合并成一条消息。

配置建议：
- “公网播放上传限速”填写外网观看时希望下载器保留的上传速度，单位 KB/s，例如 500 或 1024。
- “公网播放下载限速”是可选项；不需要插件管下载速度就留空。
- “内网或未观看上传限速”填写恢复值；留空或 0 表示解除上传限速。
